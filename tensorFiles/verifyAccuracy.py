...
FOLDERS = ["Fold 1", "Fold 2", "Fold 3", "Fold 4", "Fold 5"]
...
def main():
    ...
    for folders in FOLDERS:
        print(f"{MODEL_LOCATION}/{folders}")
        file_paths = pathlib.Path(f"{MODEL_LOCATION}/{folders}").glob("*.keras")
        print(file_paths)
        for file in file_paths:
            print(file)
            evaluate_test = run_model(file, True)
            print(evaluate_test)
            evaluate_train = run_model(file, False)
            print(evaluate_train)
            os.rename(file, f"{MODEL_LOCATION}/model_fold1_acc{evaluate_train[1]}_vacc{evaluate_test[1]}.keras")
